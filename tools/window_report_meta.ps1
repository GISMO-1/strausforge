param(
  ...
  [Parameter(Mandatory=$true)][string]$MetaJsonl,
  [int]$Step = 97,
  [int[]]$Residues = @(1, 25)
)
...
Get-Content $MetaJsonl | ForEach-Object {
  $line = $_.Trim()
  if ($line.Length -eq 0) {
    return
  }
  ...
  if ($entry.semiprime_kind -eq "p*q") {
    $semiprimeCount += 1
  }
  elseif ($entry.semiprime_kind -eq "p^2") {
    $sqCount += 1
  }
}
...
$residuesArg = ($Residues -join ",")
$py = "python tools\sample_semiprime_rate.py --n-min $NMin --n-max $NMax --step $Step --residues $residuesArg"
Write-Host ("baseline_cmd: {0}" -f $py)

$baselineOut = & python tools\sample_semiprime_rate.py --n-min $NMin --n-max $NMax --step $Step --residues $residuesArg
